...
import { isNonEmptyArray, isValidArrayIndex } from "./utils/data-types";
import { ValueEnumManager } from "./hint-data/value-enum-manager";

const hintData = new HintDataManager();
const valueEnum = new ValueEnumManager();
...
export function activate(context: ExtensionContext) {
    ...
    function reloadConfig() {
        config.reload();
        if (config.lintDirectiveKeys) window.visibleTextEditors.forEach((editor) => lintDocument(editor.document));
    ...
    function lintDocument(document: TextDocument) {
        ...
        dirs.forEach((it) => {
            ...
            if (config.customDirectiveRegexps.findIndex((it) => it.test(directiveName)) >= 0) return;
        ...
        switch (cursorContext.type) {
            case CursorType.directiveKey: {
                ...
                if (mtx) {
                    ...
                    directives = directives.filter((it) => it.directiveNameLC?.startsWith(prefix));
                }
            ...
            case CursorType.section:
                return completionItemsForSections;
            case CursorType.directiveValue: {
                const pending = getPendingText();
                if (pending.endsWith("%") && !pending.endsWith("%%")) return hintData.specifiers;
                return valueEnum.resolve(cursorContext);
            }
        }
    ...
    function provideSignatureHelp(
        ...
        for (const directive of dirs) {
            const manPage = isValidArrayIndex(directive.manPage) ? hintData.manPages[directive.manPage] : undefined;
            const docs =
                (isValidArrayIndex(directive.docsMarkdown) && hintData.docsMarkdown[directive.docsMarkdown]) || "";
            const signStrings = isNonEmptyArray(directive.signatures) ? directive.signatures : [""];

            for (let signString of signStrings) {
                if (manPage) signString += " " + manPage.title;
                signatures.push(new SignatureInformation(signString, docs));
            }
        }
