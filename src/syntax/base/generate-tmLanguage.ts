...
import { TextMateGrammarConfig, TextMateGrammarPattern, TextMateGrammarPatterns } from "./tmLanguage-types";
...
const captureKeys: Array<keyof TextMateGrammarPattern> = [
	'captures',
	'beginCaptures',
	'endCaptures',
]

export function generateTmLanguage(
    syntax: TextMateGrammarConfig,
    file: Writable
) {
    let currentTab = "";
    ...
    //#region patterns
    const patterns: TextMateGrammarPattern[] = [];
    syntax.patterns.forEach((it) =>
        Array.isArray(it) ? it.forEach((nested) => nested && patterns.push(nested)) : it && patterns.push(it)
    );
    ...
    function writePatterns(patterns: TextMateGrammarPatterns) {
        ...
        for (let i = 0; i < patterns.length; i++) {
            ...
            if (!pattern) continue;
            write("<dict>", 1);
            if (pattern.comment) write(`<!-- ${pattern.comment.replace(escapeRegexp, escapeCharFn)} -->`);
            if (pattern.include) {
            ...
            if (pattern.match) write(keyTag("match", "string", pattern.match));
            ...
            captureKeys.forEach((key) => {
                ...
                for (let j = 0; j < capKeys.length; j++) {
                    ...
                    if (typeof cap === "string") cap = { name: cap };
                    write(keyTag(capKey, "dict", keyTag("name", "string", cap.name)));
                }
                write("</dict>", -1);
            });

            if (pattern.contentName) write(keyTag("contentName", "string", pattern.contentName));
            if (pattern.patterns) writePatterns(pattern.patterns);
            write("</dict>", -1);
        }
        write("</array>", -1);
    }
